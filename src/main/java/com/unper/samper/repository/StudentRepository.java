...
public interface StudentRepository extends JpaRepository<Student, Long> {
    ...
    @Query(value = "SELECT s.* FROM public.student s WHERE s.class_id = :classId AND s.is_leader = true", nativeQuery = true)
    Optional<Student> findStudentLeaderByClass(@Param("classId") Long classId);

    @Override
    @Query(value = "SELECT s.* FROM public.student s LEFT JOIN public.user u ON u.id = s.user_id WHERE u.is_deleted = false", nativeQuery = true)
