...
public interface PresenceRepository extends JpaRepository<Presence, Long> {
    @Query("SELECT count(p)>0 FROM Presence p WHERE p.student = :student AND p.schedule = :schedule AND status ='O'")
    Boolean isCheckOut(@Param("student") Student student, @Param("schedule") Schedule schedule);

    @Query("SELECT count(p)>0 FROM Presence p WHERE p.student = :student AND p.schedule = :schedule AND status ='I'")
