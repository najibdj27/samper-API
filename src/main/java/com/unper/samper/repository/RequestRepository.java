...
public interface RequestRepository extends JpaRepository<Request, Long> {
    ...
    @Override
    @Query(value = "SELECT * FROM public.request WHERE is_deleted = false", nativeQuery = true)
    List<Request> findAll();

    @Override
    @Query(value = "SELECT * FROM public.request WHERE is_deleted = false AND id = :id", nativeQuery = true)
    Optional<Request> findById(@Param("id") Long id);

    @Override
    @Modifying
    @Transactional
    @Query(value = "UPDATE public.request SET is_deleted = true WHERE id = :id", nativeQuery = true)
